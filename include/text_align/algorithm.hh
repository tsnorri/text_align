...
namespace text_align {
	...
	inline std::uint64_t reverse_bits(std::uint64_t r)
	{
	...
	inline void reverse_bitset(boost::dynamic_bitset <std::uint64_t> &bs)
	{
		...
		{
			std::uint64_t const reversed(reverse_bits(word));
			word = reversed;
		}
